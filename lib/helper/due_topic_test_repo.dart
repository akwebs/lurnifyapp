...
class DueTopicTestRepo{
  ...
  Future<List<Map<String,dynamic>>> getDueTopicTestByStatusAndTopicAndRegister(String status,
  ...
  Future<List<Map<String,dynamic>>> getDueTopicTestByStatusAndRegister(String status, String register, String revision)async{
    ...
    String sql = "select dtt.sno as dueTopicTestSno,topic.sno as topicSno ,topic.topicName as topicName,topic.subTopic as subtopic,"
        ...
        + "from due_topic_tests as dtt " + "INNER JOIN topic on dtt.topicSno=topic.sno "
        + "INNER JOIN chapter as c on c.sno=topic.chapterSno " + "INNER JOIN unit as u on u.sno=c.unitSno "
        + "INNER JOIN subject as sub on sub.sno=u.subjectSno " + "where registerSno='$register' and `status`='$status'";
