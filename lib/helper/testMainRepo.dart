...
class TestMainRepo {
  ...
  insertIntoTestMain(TestMain testMain,txn) async {
    try {
      ...
      txn.rawInsert(
          "insert into test_main values(${testMain.sno},'${testMain.enteredBy}','${testMain.enteredDate}',"
  ...
  Future<List<Map<String, dynamic>>> findByTopicSetAndTopic(
      String topicSet, String topic) async {
    Database db = await dbHelper.database;
    String sql =
        "select * from test_main where topic=$topic and topicSet=$topicSet";
    print(sql);
    var result = db.rawQuery(sql);
    return result;
  }

  Future<List<Map<String, dynamic>>> findByTopicAndRegister(
      String topic, String register) async {
    ...
    List<Map<String, dynamic>> find =
        await topicTestResultRepo.findTopByRegisterAndTopicOrderBySnoDesc(register, topic);
    ...
    if (find == null || find.isEmpty) {
      ...
      test =  findByTopicSetAndTopic('1', topic);
        ...
        double.parse(find[0]['testPercent']) < 70) {
      int setNo = 0;
    ...
    } else if (double.parse(find[0]['testPercent']) >= 70 &&
        double.parse(find[0]['testPercent']) < 90) {
      int setNo = 0;
      ...
      test =  findByTopicSetAndTopic(setNo.toString(), topic);
    } else if (double.parse(find[0]['testPercent']) >= 90 &&
        double.parse(find[0]['testPercent']) <= 100) {
      int setNo = 0;
