  ...
  _getHomePageData() async {
    try {
      ...
      if (list.isEmpty) {
        ...
        for (var a in list) {
          ...
          if (a['dataSynced'] == null ||
            ...
            DailyTaskCompletionRepo dailyTaskCompletionRepo =
                new DailyTaskCompletionRepo();
            List<Map<String, dynamic>> dailyResult =
                await dailyTaskCompletionRepo.getNewDailyTaskCompletion();
            for (int i = 0; i < dailyResult.length; i++) {
              batch.rawUpdate(
                  "update daily_task_completion set onlineStatus='old' where sno=${dailyResult[i]['sno']}");
      ...
      await database.transaction((txn) async {
        ...
        String sql4 =
            "select (sum(correctQuestion)/sum(totalQuestion)) as testPercent from topic_test_result";
      ...
      await FirebaseFirestore.instance
          ...
          .add({'register': registerSno, 'lastLoginTime': lastLoginTime});
