  ...
  Future _testSubmit() async {
    ...
    if (testType == "topicTest") {
      ...
      if(_testPercent>=50){
        ...
        List<Map<String,dynamic>> list2=await dueTopicTestRepo
            .getDueTopicTestByStatusAndTopicAndRegister('INCOMPLETE', sno, sp.getString("studentSno"));
        if(list2.isNotEmpty){
          dueTopicTestRepo.updateDueTopicTest(sno);
          FirebaseFirestore.instance.collection('dueTopicTest')
            .where('status',isEqualTo: 'INCOMPLETE')
            .where('topicSno',isEqualTo: sno)
            .where('registerSno',isEqualTo: sp.getString("studentSno")).get().then((QuerySnapshot snapshot) {
    ...
    if (result>0) {
      ...
      Navigator.of(context).pushReplacement(MaterialPageRoute(
        builder: (context) => TestResult(_correctQuestion, _wrongAnswer,
            _resultNumber, _testData, _answerMap, _bookmarkMap,
            {}),//yha pr response ko hataye hai
      ));
    } else {
      toastMethod("Failed.");
    }
