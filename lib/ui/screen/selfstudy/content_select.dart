...
class _ContentSelectState extends State<ContentSelect> {
  ...
  Future _getSubjects() async {
    ...
    var a = await db.rawQuery('select * from subject where courseSno=${sp.getString('courseSno')}');
    for (var b in a) {
      Subject subject = new Subject();
      subject.sno = b['sno'];
      subject.subjectName = b['subjectName'];
      var sql = await db.rawQuery("select * from unit where subjectSno='${b['sno']}'");
      List<UnitDtos> u = [];
      for (var c in sql) {
        ...
        var sql2 = await db.rawQuery("select * from chapter where unitSno=${c['sno']}");
        ...
        for (var d in sql2) {
          ...
          var sql3 = await db.rawQuery("select * from topic where chapterSno=${d['sno']}");
          List<TopicDtos> t = [];
          for (var e in sql3) {
                  ...
                  child: Center(
                    child: InkWell(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(color: whiteColor, fontSize: 50, fontWeight: FontWeight.bold),
                      ),
                    ),
                  ),
                );
