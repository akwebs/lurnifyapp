...
import 'package:lurnify/ui/screen/rankBooster/rankbooster-instruction.dart';
...
class _CreateYourOwnTestState extends State<CreateYourOwnTest> {
  ...
  Future _getCompletedChapters() async {
    try {
      ...
      var url = baseUrl +
          "getCompletedChaptersByReg?regSno=" +
                    ...
                    Padding(
                      ...
                      child: Container(
                        ...
                        child: selectedSwitchOption == "Numerical"
                            ? Text(
                                "Total time will be " +
                                    (_selectedSubjectMap.length *
                                            int.parse(_noOfQuestion) *
                                            2)
                                        .toString(),
              ...
              child: Container(
                ...
                child: ExpansionTile(
                  ...
                  children: [
                    ListView.builder(
                      itemCount:
                          _completedChapters[i]['completedChapSubs'].length,
                      primary: false,
                      shrinkWrap: true,
                      scrollDirection: Axis.vertical,
                      itemBuilder: (context, j) {
                        _checkBoxMap.putIfAbsent(
                            _completedChapters[i]['completedChapSubs'][j]
                                ['sno'],
                            () => false);
                        return Row(
                          children: [
                            Expanded(
                              child: CheckboxListTile(
                                value: _checkBoxMap[_completedChapters[i]
                                    ['completedChapSubs'][j]['sno']],
                                title: Text(_completedChapters[i]
                                    ['completedChapSubs'][j]['chapterName']),
                                onChanged: (newValue) {
                                  setState(() {
                                    _checkBoxMap[_completedChapters[i]
                                            ['completedChapSubs'][j]['sno']] =
                                        newValue;
                                    if (newValue) {
                                      _addToSubjectMap(
                                          _completedChapters[i]['sno'],
                                          _completedChapters[i]
                                              ['completedChapSubs'][j]['sno']);
                                    } else {
                                      _removeFromSubjectMap(
                                          _completedChapters[i]['sno'],
                                          _completedChapters[i]
                                              ['completedChapSubs'][j]['sno']);
                                    }
  ...
  _navigateToInstructionPage() {
    ...
    if (selectedSwitchOption == "Numerical") {
      testDuration = (_selectedSubjectMap.length * int.parse(_noOfQuestion) * 2)
          .round()
          .toString();
    } else {
      testDuration =
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(
      builder: (context) => RankboosterInstruction(
          testDuration,
          "4",
          _selectedSubjectMap,
